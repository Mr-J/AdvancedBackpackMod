...
public class GuiBackpackShared extends GuiContainer {
		
	//ContainerBackpackBase myContainer;
	ContainerBackpackShared inventoryContainer;
	String containerName;
	
	public GuiBackpackShared(InventoryBackpackGeneral backpackInventory, IInventory containerInventory, String blockName)
	{
		super(new ContainerBackpackShared(backpackInventory, containerInventory));
		//myContainer = (ContainerBackpackBase) super.inventorySlots;
		inventoryContainer = (ContainerBackpackShared) super.inventorySlots;
		containerName = blockName;
		...
		this.xSize = 14 + (Math.max(inventoryContainer.invBackpackCol, inventoryContainer.invContainerCol) * 18);
		this.ySize = 38 + (inventoryContainer.invBackpackRow + inventoryContainer.invContainerRow) * 18;
		if(inventoryContainer.invBackpackRest > 0)
		{
			this.ySize = this.ySize + 18;
		}
		if(inventoryContainer.invContainerRest > 0)
		{
			this.ySize = this.ySize + 18;
		}
	}
	...
	{
	    //Since most containers lack a real name this would look ugly, so its not used
	    //fontRenderer.drawString(containerName, 8, 6, 4210752);
		if (inventoryContainer.invContainerRest > 0)
		{
	        fontRenderer.drawString("Backpack ("+ inventoryContainer.invBackpackSize + " Slots)", 8, (inventoryContainer.invContainerRow + 1) * 18 + 20, 4210752);
	    }
		else
		{
		    fontRenderer.drawString("Backpack ("+ inventoryContainer.invBackpackSize + " Slots)", 8, inventoryContainer.invContainerRow * 18 + 20, 4210752);
		}
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) 
	{
		...
		int maxColumns = Math.max(inventoryContainer.invBackpackCol, inventoryContainer.invContainerCol);
		//int rows = myContainer.invRow;
		int startX = (this.width - this.xSize)/2;
		int startY = (this.height - this.ySize)/2;
		
		drawContainerTop(startX, startY, maxColumns);
		drawContainerMiddle(startX, startY + 17, maxColumns, inventoryContainer.invBackpackRow, inventoryContainer.invContainerRow);
		int offset = 0;
		if (inventoryContainer.invBackpackRest > 0)
		{
			offset = offset + 1;
		...
		drawContainerBottom(startX, startY + 31 + (inventoryContainer.invBackpackRow + 
								inventoryContainer.invContainerRow + offset) * 18, maxColumns);
	}
	
	private void drawContainerMiddle(int x, int y, int columns, int rowsBackpack, int rowsContainer)
	{
		int currentX = x;
		int currentY = y;
		//int rest = myContainer.invSize - (myContainer.invCol * myContainer.invRow);
		
		for (int i = 1; i <= rowsContainer; i++)
		{
			drawContainerSlotLine(x, currentY, columns - inventoryContainer.invContainerCol, inventoryContainer.invContainerCol);
			currentY = currentY + 18;
		}
		if (inventoryContainer.invContainerRest > 0)
		{
			drawContainerSlotLine(x, currentY, columns - inventoryContainer.invContainerRest, inventoryContainer.invContainerRest);
			currentY = currentY + 18;
		}
		...
		for (int i = 1; i <= rowsBackpack; i++)
		{
			drawContainerSlotLine(x, currentY, columns - inventoryContainer.invBackpackCol, inventoryContainer.invBackpackCol);
			currentY = currentY + 18;
		}
		if (inventoryContainer.invBackpackRest > 0)
		{
			drawContainerSlotLine(x, currentY, columns - inventoryContainer.invBackpackRest, inventoryContainer.invBackpackRest);
			currentY = currentY + 18;
		}
